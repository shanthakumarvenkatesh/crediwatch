...
const Medium = () => {
  ...
  let mediumParties = parties
    .filter((data) => {
      return data.score > 5&&data.score<=10;
    })
    .map((data) => {
      return data;
    });
  ...
  const currentparties = mediumParties.slice(indexofFirstParty, indexofLastParty);
  ...
  window.onload = () => {
    ...
    for (let i = 0; i <= mediumParties.length; i++) {
        ...
        <div className='container'>
          ...
          <div className='d-flex justify-content-between flex-lg-row flex-column'>
            <p id='hidden' className='mb-0 fw-bold mt-3 mt-lg-0 mb-3 mb-lg-0'>
              Displaying {partiesPerPage} parties in {mediumParties.length}
            </p>
            <div className='overflow-scroll'>
              <Pagination
                partiesPerPage={partiesPerPage}
                totalparties={mediumParties.length}
                paginate={paginate}
              />
            </div>
          </div>
